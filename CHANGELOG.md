...
- Chrome windows no longer end up at an in-between size or position when tiled, especially across displays. Chrome animates Accessibility-initiated frame changes over a few hundred milliseconds; Tiley's verification read the frame immediately, mistook the mid-animation value for a refusal, and fired rescue paths (a top-left bounce, repeated position re-sets) that redirected the in-flight animation, leaving the window at an intermediate frame. Every verification point now waits for the frame to stop moving before judging, and the primary-screen path gained a final settled size assertion it previously lacked. Apps that apply frames synchronously see no added latency.
